...
class poopoonet:
    ...
    def build_model(self, input_shape, num_output):
        ...
        with slim.arg_scope([slim.layers.convolution, slim.layers.fully_connected],
            ...
            net = slim.layers.convolution(self.inputs, num_outputs=20, kernel_size=3, scope='conv1')
            ...
            net = slim.layers.fully_connected(net, num_output, activation_fn=tf.nn.softmax, weights_regularizer=None)
            self.net = net

    def train_model(self, learning_rate, train_data, validation_data, validation_epoch, outdir=None):
        self.predictions = tf.placeholder(tf.float32, shape=[1, self.num_output])
        predict_loss = tf.losses.softmax_cross_entropy(self.net, self.predictions)
        ...
        for image, label in zip(batch_image, batch_label):
            _, loss = self.sess.run([self.trainer, total_loss], feed_dict={self.inputs:image, self.predictions:label})
    ...
    def infer_model(self, data):
        ...
        for image, label in zip(batch_images, batch_labels):
             predictions.append(self.sess.run(self.net, feed_dict={self.inputs:image})[0])
